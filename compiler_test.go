...
func TestBasicMatch(t *testing.T) {
	...
	if !rs.Rows[0]["1"].Get().(graph.Node).GetLabels().HasAll("t1", "t2") {
	...
	if !(len(rs.Rows) == 2 && rs.Rows[0]["1"].Get().(graph.Node).GetLabels().Has("t1") &&
		rs.Rows[1]["1"].Get().(graph.Node).GetLabels().Has("t1")) {
		t.Errorf("Expecting to see two rows with t1, t2")
	}
	rs = runTestMatch(t, "match (n:t2) return n", g)
	if !(len(rs.Rows) == 1 && rs.Rows[0]["1"].Get().(graph.Node).GetLabels().Has("t1")) {
	...
	if !(len(rs.Rows) == 2 && rs.Rows[0]["1"].Get().(graph.Node) == n3 && rs.Rows[1]["1"].Get().(graph.Node) == n3) {
